        ...
        if ($root != 0) {
            if (strpos($root, ',')) {
                ...
                if ($parentCode) {
                    F('parents', NULL);
                    $parents = F('parents');
                    $parents = $parents . "" . $root . '-' . $parentCode . '-';
                    F('parents', $parents);
...
//        dump(F('parents'));
        ...
        $show = $page->show(); //分页模板
//        var_dump($where);
        $files = $file->where($where)->limit($page->firstRow . ',' . $page->listRows)->order('uploadtime desc')->select();
...
//        dump($_FILES["upfile"]);
        $text_name = $this->_post('text_name');
        ...
        $up_obj = new UploadFile(); // 实例化上传类
        $up_obj->maxSize = 104857600; // 设置附件上传大小,100M
//        $up_obj->allowExts = array('jpg', 'gif', 'png', 'jpeg'); // 设置附件上传类型
        $up_obj->savePath = './Public/Uploads/'; // 设置附件上传目录
        if (!$up_obj->upload()) {// 上传错误提示错误信息
            $this->error($up_obj->getErrorMsg());
        } else {// 上传成功 获取上传文件信息
            $info = $up_obj->getUploadFileInfo();
        }
...
//        $data['filename'] = $_FILES["upfile"]['name'];//不使用原因，当上传重复文件时需要加（2）
        ...
        if($_POST['coverUpload']=="cover"){
            ...
            rename('./Public/Uploads/' . $info[0]['savename'], './Public/Uploads/' . $name);
            $this->redirect('Index/home');
            exit();
        }
